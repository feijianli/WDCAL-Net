...
from ..network.blocks.prior import  ideal_affinity_matrix, prior_losses,C1_ideal_affinity_matrix,C1_prior_losses
...
class SegmentationMultiProcess(object):
    ...
    def _train(self, epoch):
        ...
        for index, (images, masks) in enumerate(self.train_loader):
            ...
            if self.cfg.FINE_MODEL.DEEP_SUPERVISION:
                ...
                output_seg = output_seg_list[0]
                loss_func = SegLoss(loss_func=self.cfg.TRAINING.LOSS, activation=self.cfg.TRAINING.ACTIVATION)
                seg_loss1 = loss_func(output_seg, masks, is_average=True)

                # output_seg,map,C1_map = self.model(images)   #c_prior
 ...
 # prior loss
                ...
                C1_ideal_masks = C1_ideal_affinity_matrix(masks)
                ...
                prior_loss = prior_losses(output_seg_list[1], ideal_masks)
                C1_prior_loss = C1_prior_losses(output_seg_list[2], C1_ideal_masks)
                # prior_loss = prior_losses(map,ideal_masks)
                # C1_prior_loss = prior_losses(C1_map, C1_ideal_masks)
                # print(prior_loss)
                # print(C1_prior_loss)
                # print(seg_loss)
                # seg_loss = seg_loss1
                # seg_loss = seg_loss1 + 10*prior_loss
                seg_loss = seg_loss1 + prior_loss + 20*C1_prior_loss
            ...
            if self.is_print_out:
                if index > 0 and index % self.cfg.TRAINING.SAVER.SAVER_FREQUENCY == 0:
                    ...
                    self.logger.info('PriorLoss:{:.6f}'.format(prior_loss.item()))
                    self.logger.info('C1_PriorLoss:{:.6f}'.format(C1_prior_loss.item()))
                    self.logger.info('SegLoss1:{:.6f}'.format(seg_loss1.item()))
        ...
        for images, masks in self.val_loader:
            ...
            with torch.no_grad():
                if self.cfg.FINE_MODEL.DEEP_SUPERVISION:
                    ...
                    output_seg_list = self.model(images)
                    output_seg = output_seg_list[0]
                    # output_seg,map,C1_map = self.model(images)     # prior loss
                    # output_seg = self.model(images)
    ...
    def do_test(self, fold=1):
        ...
        for batch_idx, data_dict in enumerate(test_dataset):
            ...
            with torch.no_grad():
                output_seg_list = self.fine_model(crop_image)
                crop_image = output_seg_list[0]

                # crop_image,map,C1_map = self.fine_model(crop_image)
