...
from BaseSeg.network.blocks.prior import PriorConvBlock,C1_PriorConvBlock, Bottleneck
from BaseSeg.network.blocks.Attention_block import GlobalBlock
...
class EfficientSegNet(nn.Module):
    ...
    def __init__(self, cfg=None):
        ...
        #attentionBlock
        self.gblock_0 = GlobalBlock(num_channel[0])
        self.gblock_1 = GlobalBlock(num_channel[1])
        self.gblock_2 = GlobalBlock(num_channel[2])
        self.gblock_3 = GlobalBlock(num_channel[3])
        self.gblock_4 = GlobalBlock(num_channel[4])
        ...
        # self.bottleneck = _ConvINReLU3D(num_class * 3, num_class * 4, kernel_size=1, stride=1, padding=0, p=0)
        # self.final2 = nn.Conv3d(num_class * 4, num_class, kernel_size=1,bias=False )
        ...
        self.final = nn.Conv3d(num_channel[0], num_class, kernel_size=1)

        # self.final = _ConvINReLU3D(num_channel[0], num_class, kernel_size=1, stride=1, padding=0, p=0.2)

        self.prior = PriorConvBlock(num_class, num_class )
        self.C1_prior = C1_PriorConvBlock(num_class, num_class)

        self.bottleneck = Bottleneck(num_class)
    ...
    def forward(self, x):
        ...
        x = self.gblock_0(x)
        x1_0 = self.conv1_0(x)
        x1_0 = self.gblock_1(x1_0)
        x2_0 = self.conv2_0(x1_0)
        x2_0 = self.gblock_2(x2_0)
        x3_0 = self.conv3_0(x2_0)

        x3_0 = self.gblock_3(x3_0)
        x4_0 = self.conv4_0(x3_0)
        x4_0 = self.gblock_4(x4_0)
        # x4_0 = self.context_block(x4_0)

        x3_1 = self.conv3_1(self.up(self.trans_4(x4_0)) + x3_0)
        x3_1 = self.gblock_3(x3_1)

        if self.is_dynamic_empty_cache:
            del x4_0
            torch.cuda.empty_cache()
        x2_1 = self.conv2_2(self.up(self.trans_3(x3_1)) + x2_0)
        x2_1 = self.gblock_2(x2_1)
        if self.is_dynamic_empty_cache:
            del x3_1
            torch.cuda.empty_cache()
        if self.auxiliary_task:
            out_1 = self.final1(x2_1)

        x1_1 = self.conv1_3(self.up(self.trans_2(x2_1)) + x1_0)
        x1_1 = self.gblock_1(x1_1)
        if self.is_dynamic_empty_cache:
            del x2_1
            torch.cuda.empty_cache()

        x = self.conv0_4(self.up(self.trans_1(x1_1)) + x)
        if self.is_dynamic_empty_cache:
            del x1_1
            torch.cuda.empty_cache()
        ...
        # prior module
        # out, context_prior_map = self.prior(x)
        out_list = self.prior(x)

        # # C1_out, C1_context_prior_map = self.C1_prior(x)
        C1_out_list = self.C1_prior(x)

        x = torch.cat([x, out_list[0],C1_out_list[0]], dim=1)
        x = x.type(torch.cuda.HalfTensor)
        x = self.bottleneck(x)


        # x = out_list[0] + C1_out_list[0]
        # x = x
        ...
        if self.auxiliary_task:
            ...
            # return x,context_prior_map,C1_context_prior_map
            return [x, out_list[1], C1_out_list[1]]
            # return x

        # if self.is_postprocess:
        #     x = self.output-buzhi(x, out_size)
        #
        # if self.auxiliary_task:
        #     return [x, out_1]
        # else:
